...
namespace kntodev\simplemessage\helpers;
...
class TimeElapsed {
    ...
    public static function timeInterval($interval, $append = null, $human = true)
    {
        ...
        if ($append === null) {
            $append = ' ' . Yii::t('modules/messages', 'ago');
        }
        if ($human) {
            if ($interval <= 0) {
                $elapsed = Yii::t('modules/messages', 'a moment') . $append;
            } elseif ($interval < 60) {
                $elapsed = Yii::t('modules/messages', '{n, plural, one{one second} other{# seconds}}',
                        ['n' => $interval]) . $append;
            } elseif ($interval >= 60 && $interval < $intervals['hour']) {
                $interval = floor($interval / $intervals['minute']);
                $elapsed = Yii::t('modules/messages', '{n, plural, one{one minute} other{# minutes}}',
                        ['n' => $interval]) . $append;
            } elseif ($interval >= $intervals['hour'] && $interval < $intervals['day']) {
                $interval = floor($interval / $intervals['hour']);
                $elapsed = Yii::t('modules/messages', '{n, plural, one{one hour} other{# hours}}', ['n' => $interval]) . $append;
            } elseif ($interval >= $intervals['day'] && $interval < $intervals['week']) {
                $interval = floor($interval / $intervals['day']);
                $elapsed = Yii::t('modules/messages', '{n, plural, one{one day} other{# days}}', ['n' => $interval]) . $append;
            } elseif ($interval >= $intervals['week'] && $interval < $intervals['month']) {
                $interval = floor($interval / $intervals['week']);
                $elapsed = Yii::t('modules/messages', '{n, plural, one{one week} other{# weeks}}', ['n' => $interval]) . $append;
            } elseif ($interval >= $intervals['month'] && $interval < $intervals['year']) {
                $interval = floor($interval / $intervals['month']);
                $elapsed = Yii::t('modules/messages', '{n, plural, one{one month} other{# months}}',
                        ['n' => $interval]) . $append;
            } elseif ($interval >= $intervals['year']) {
                $interval = floor($interval / $intervals['year']);
                $elapsed = Yii::t('modules/messages', '{n, plural, one{one year} other{# years}}', ['n' => $interval]) . $append;
            }
        } else {
